...
Global_traj_class::Global_traj_class(ros::NodeHandle nh, ros::NodeHandle private_nh)
{
    ...
    while (set_goals)
    {
        ...
        if (shared_goal == nullptr)
        {
            if (goal_nodes_.empty())
            {
                ...
                default_way_points << 25.0 , 0,
                                      25.0  , 4.5;
                                    //   9.0-1.3 , -7.5;

                for (unsigned int ii = 0; ii < default_way_points.rows(); ii++)
                {
                    goal_node_ << default_way_points(ii,0), default_way_points(ii,1);
